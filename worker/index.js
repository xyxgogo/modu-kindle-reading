...
function accountCookie(token) {
  return `${COOKIE_ACCOUNT}=${encodeURIComponent(token)}; Path=/; Max-Age=${60 * 60 * 24 * 30}; HttpOnly; Secure; SameSite=Lax`;
}

function clearAccountCookie() {
  return `${COOKIE_ACCOUNT}=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Lax`;
}

function fontScaleConfig(value) {
  const configs = {
    standard: { label: "标准", pixels: 32, target: 560 },
    large: { label: "大", pixels: 38, target: 430 },
    extra_large: { label: "加大", pixels: 44, target: 330 },
    extra_extra_large: { label: "加加大", pixels: 50, target: 250 },
  };
...
<body>
<main>
  <header class="brand"><strong>墨读</strong><span class="muted">Kindle 家庭阅读与单词学习站</span></header>
  ${body}
  ${nav ? `<nav>${nav}</nav>` : ""}
</main>
${script}
...
async function accountLogin(request, env, url, deviceSession) {
  ...
  if (request.method === "POST") {
    ...
    const digest = account ? await passwordDigest(password, account.password_salt, account.password_iterations) : "";
    if (!account || account.status !== "active" || !safeEqual(digest, account.password_hash)) {
...
async function accountRegister(request, env, url, deviceSession) {
  ...
  if (!legacyUser) {
    statements.push(
      ...
      env.DB.prepare(`INSERT INTO user_preferences
        (id, user_id, reading_font_size, reading_font_scale, created_at, updated_at)
        VALUES (?, ?, 'medium', 'standard', datetime('now'), datetime('now'))`)
  ...
  statements.push(
    env.DB.prepare(`INSERT INTO accounts
      (id, user_id, username, normalized_username, password_salt, password_hash, password_iterations, role, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'child', 'active', datetime('now'), datetime('now'))`)
...
async function parentChildren(request, env, account, url) {
  ...
  return htmlResponse(layout({
    ...
    body: `${parentNavigation()}<h1>孩子/学生设定</h1>
      ...
      <table><thead><tr><th>ID</th><th>注册状态</th><th>绑定时间</th></tr></thead>
      <tbody>${rows || '<tr><td colspan="3">尚未绑定孩子/学生。</td></tr>'}</tbody></table>`,
...
async function routeParent(request, env, url) {
  ...
  if (url.pathname === "/parent/books" && request.method === "GET") return parentBooks(request, env, account, url);
...
async function homePage(env, url, session, user, account) {
  ...
  return htmlResponse(layout({
    title: isParent ? `家长 ${account.username} 的主页` : `${user.display_name}的个人主页`,
    refresh: true,
    body: `${weather}
      ${notice ? `<div class="notice">${escapeHtml(notice)}</div>` : ""}
      <div class="current-user">${isParent
        ? `尊敬的家长 ${escapeHtml(account.username)}`
        : `孩子：${escapeHtml(user.display_name)}（ID：${escapeHtml(account.username)}）`}</div>
      <h1>${isParent ? `尊敬的家长 ${escapeHtml(account.username)}` : `你好，${escapeHtml(user.display_name)}`}</h1>
      <a class="button" href="/k/books">阅读</a>
      ${isParent ? "" : '<a class="button" href="/k/words">单词</a>'}
      <a class="button" href="/parent">${account.role === "parent" ? "家长功能" : "申请成为家长"}</a>
      ${isParent ? "" : `<section class="card">
        <h2>今日概况</h2>
        <p>今日完成题数：${Number(stats?.today_attempts || 0)}<br>
        待复习错题：${Number(stats?.pending_mistakes || 0)}</p>
      </section>`}`,
    nav: `<a href="/k/settings">显示设置</a> | <a href="/k/logout">退出登录</a>`,
  }));
}
...
async function settingsPage(request, env, session, user) {
  if (request.method === "POST") {
    ...
    if (!["standard", "large", "extra_large", "extra_extra_large"].includes(fontSize)) {
      return errorPage(400, "设置无效", "请选择标准、大、加大或加加大字号。");
    }
    await env.DB.prepare(`
      UPDATE user_preferences SET reading_font_scale = ?, updated_at = datetime('now') WHERE user_id = ?
    `).bind(fontSize, user.id).run();
    return redirect("/k/settings?saved=1");
  }
  const pref = await env.DB.prepare(`
    SELECT reading_font_scale FROM user_preferences WHERE user_id = ? LIMIT 1
  `).bind(user.id).first();
      ...
      <form method="post" action="/k/settings">
        ...
        <fieldset>
          ...
          ${["standard", "large", "extra_large", "extra_extra_large"].map((value, index) => `
            <label class="radio-line"><input type="radio" name="reading_font_scale" value="${value}" ${current === value ? "checked" : ""}> ${["标准", "大", "加大", "加加大"][index]}</label>
          `).join("")}
        </fieldset>
        <input type="submit" value="保存设置">
      </form>`,
    nav: `<a href="/k/home">个人主页</a> | <a href="/k/logout">退出登录</a>`,
  }));
}
...
async function adminUsers(env, identity, url) {
  const result = await env.DB.prepare(`
    SELECT u.id, u.display_name, a.username, a.role, a.status, a.created_at,
  ...
  const rows = (result.results || []).map((user) => `<tr>
    <td>${user.id ? `<a href="/admin/users/${encodeURIComponent(user.id)}">${escapeHtml(user.username)}</a>` : escapeHtml(user.username)}</td>
  ...
  return htmlResponse(layout({
    ...
    body: `${adminNavigation()}<h1>账户管理</h1>
      ...
      <table><thead><tr><th>用户名</th><th>角色</th><th>状态</th><th>阅读进度</th><th>答题</th><th>错题</th></tr></thead>
      <tbody>${rows || '<tr><td colspan="6">暂无注册账户。</td></tr>'}</tbody></table>`,
...
async function kindleBooks(env, user, account) {
  const result = await env.DB.prepare(`
  ...
  `).bind(user.id, account.account_id, account.normalized_username).all();
  ...
  return htmlResponse(layout({
    title: "阅读",
    body: `<div class="current-user">孩子：${escapeHtml(user.display_name)}</div>
      <h1>阅读</h1>${cards || '<p class="warning">书架暂时为空。绑定家长上传的读物经管理员审核通过后会显示在这里。</p>'}`,
...
async function kindleRead(env, user, session, account, bookId, chapterId, pageNumber) {
  const pref = await env.DB.prepare(`SELECT reading_font_scale FROM user_preferences WHERE user_id = ? LIMIT 1`).bind(user.id).first();
  const scale = pref?.reading_font_scale || "standard";
  const config = fontScaleConfig(scale);
  const book = await env.DB.prepare(`
  ...
  const chapters = (chapterResult.results || []).map((chapter) => ({
    ...chapter,
    pages: paginateChapter(chapter.body, config.target),
  }));
  ...
  await env.DB.prepare(`
    INSERT INTO reading_progress
      (id, user_id, book_id, chapter_id, page, font_size, reading_font_scale, last_read_at, created_at, updated_at)
  ...
  `).bind(crypto.randomUUID(), user.id, bookId, chapterId, pageNumber, scale).run();
  ...
  const nextPage = pageNumber < chapter.pages.length
    ? `/k/read/${bookId}/${chapterId}/${pageNumber + 1}`
  ...
  return htmlResponse(layout({
    ...
    body: `<div class="current-user">孩子：${escapeHtml(user.display_name)}</div>
      <p class="muted">${escapeHtml(book.title)} · 字号：${escapeHtml(config.label)}</p>
      <h1>${escapeHtml(chapter.title)}</h1>
      <article style="font-family:Georgia,'Times New Roman',serif;font-size:${config.pixels}px;line-height:1.72">${novelParagraphs(pageBody)}</article>
      <p class="muted">本章第 ${pageNumber} / ${chapter.pages.length} 页　全书约 ${progress}%</p>`,
    nav: `${prevPage ? `<a href="${escapeHtml(prevPage)}">上一页</a>` : "上一页"} | <a href="/k/book/${escapeHtml(bookId)}">目录</a> | ${nextPage ? `<a href="${escapeHtml(nextPage)}">下一页</a>` : "下一页"}<a class="home-link" href="/k/home">个人主页</a>`,
  }));
}
...
async function routeKindle(request, env, url) {
  ...
  if (readMatch && request.method === "GET") return kindleRead(env, user, accountSession, account, decodeURIComponent(readMatch[1]), decodeURIComponent(readMatch[2]), Math.max(1, Number.parseInt(readMatch[3], 10)));
  if (url.pathname === "/k/words" && request.method === "GET") return kindleWordsHome(env, user, accountSession);

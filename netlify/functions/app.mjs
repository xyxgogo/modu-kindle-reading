...
async function legacyMigrationAuth(request, sqlite) {
  if (request.method !== "POST") return new Response(null, { status: 405, headers: { allow: "POST" } });
  const expectedSecret = runtimeValue("LEGACY_MIGRATION_SECRET");
  if (!safeSecretEqual(request.headers.get("x-modu-migration-secret") || "", expectedSecret)) {
  ...
  const username = String(body?.username || "").normalize("NFKC").trim().toLocaleLowerCase("en-US");
  const password = String(body?.password || "");
  if (!username || password.length < 6 || password.length > 72) return new Response(null, { status: 401 });
  ...
  const digest = await derivePassword(password, account.password_salt, Number(account.password_iterations), 32, "sha256");
  const expectedHash = Buffer.from(String(account.password_hash), "hex");
...
async function handleRequest(request) {
  ...
  try {
    ...
    if (new URL(request.url).pathname === "/__modu_migration_auth") {
      return await legacyMigrationAuth(request, sqlite);
    }
